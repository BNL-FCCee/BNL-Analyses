...
def runPlots(var,sel,param,hsignal,hbackgrounds,extralab,splitLeg):
    ...
    if(splitLeg):
        ...
        leg2 = ROOT.TLegend(1,0.6-legsize2, 1.12, 0.62)
        leg2.SetFillColor(0)
        leg2.SetFillStyle(0)
        leg2.SetLineColor(0)
    ...
    leg.SetTextSize(param.LegendTextSize)
    leg.SetTextFont(42)
...
def drawStack(name, ylabel, legend, leftText, rightText, formats, directory, logY, stacksig, histos, colors, ana_tex, extralab, scaleSig, customLabel, nsig, nbkg, legend2=None, yields=None):
    ...
    #maxScaleLog = 2000.
    if logY:
        highY=maxScaleLog*maxh/ROOT.gPad.GetUymax()
        threshold=0.5
        if (not stacksig) and nbkg==0:
        ...
        if (not stacksig) and nbkg==0:
            hStackSig.SetMaximum(maxScale*maxh)
            hStackSig.SetMinimum(0.)
        else:
            hStack.SetMaximum(maxScale*maxh)
            hStack.SetMinimum(0.)
